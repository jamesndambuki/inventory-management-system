...
let woofer = createProduct(
    'sayonapps black subwoofer',
    'electronics',
    1757678778,
    234,
    'sayonapps ltd', ['img1.png', 'img2.png', 'img3.png'],
    2022,
);
...
const createAProduct = function() {
    ...
    const details = { name, SKUNumber, price, manufacturer, description, category };
...
const DELETE = confirm('Do you want to delete a product?');

if (DELETE == true) {
    let deleteAProduct = prompt('Enter SKU number for product you want to delete');

    for (let product of products) {

        if (deleteAProduct == product.SKUNumber) {
            alert(`Product found \n${JSON.stringify(product)}`);
            let proceedDelete = confirm('do you want to delete the product?');
...
if (EDIT == true) {
    let editAProduct = prompt('Enter SKU number for product you want to Edit or update');

    for (let product of products) {
        if (editAProduct == product.SKUNumber) {
            alert(`Product found \n${JSON.stringify(product)}`);

            let proceedEdit = confirm('do you want to edit the product?');

            if (proceedEdit == true) {
                let detailEdit = prompt('Enter the detail you want to change to update');
...
if (SEARCH == true) {
    ...
    for (let product of products) {
        let productSearch = prompt('Enter product name');
